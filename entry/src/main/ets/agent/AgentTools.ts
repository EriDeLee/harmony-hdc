...
export function buildTools(includeScreenshot: boolean): ApiTool[] {
  ...
  tools.push({
    ...
    description:
      ...
      // 时间那行是 2026-08-17 加的，来由是一次真机故障：任务说「明天」，它点了今天
      // （提示词里从头到尾没有日期，只能猜）。这句解释不能省 —— 表头新增一行就欠一句。
      '第一行是当前日期时间，「今天/明天/上周」这类说法一律以它为准，不要凭记忆推算。\n' +
      ...
      // 全角括号这一族标签（（无文字·Stack）/（可滚动区域·List）/（容器·Stack））是我们
      // 替没有文字的节点起的名字，界面上并不存在这些字。约定不解释就会被读成界面内容 ——
      // 半角括号那条（hint）就是这么栽过一次的。
      //
      // （无文字·类型）这一类是新加的，来由：日历待办的勾选圈是一个只画了个圆的可点
      // Stack，四个取名字段全空，以前整个不进清单，于是 tap 够不到、只能报坐标去 click，
      // 而坐标又落在行元素外面被拦 —— 模型连点三轮都点进了「编辑日程」。
      '全角括号包起来的标签是我们替没有文字的控件起的名字，不是界面上的字：' +
      '（无文字·类型）能点，多半是画出来的图标、勾选圈、开关，直接 tap 就行；' +
      '（可滚动区域·类型）和（容器·类型）是布局容器。\n' +
      spanNote +
  ...
  if (includeScreenshot) {
    ...
    tools.push({
      ...
      description:
        ...
        // 这句原来是「坐标必须落在 index 那个元素里，否则不会点出去」。**那条规则已改**：
        // 落在别的元素里仍然拦（那是真矛盾），落在没有元素的空白处照点 —— 画出来的
        // 东西本来就不在树上，那一拦曾把唯一正确的坐标判成错的（见 clickInside 的注释）。
        '坐标落进别的元素里会被拦下来、不点；落在没有任何元素的地方会照点。\n' +
        '点完会附一张新截图。',
      input_schema: {
        type: 'object',
        properties: {
